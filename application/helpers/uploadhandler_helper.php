...
{   
    ...
    function __construct($options = null, $initialize = true, $error_messages = null) {
        $this->options = array(
            ...
            'upload_dir' => dirname($this->get_server_var('SCRIPT_FILENAME')).'/assets/files/',
        ...
        $this->CI->load->model('uploader/uploader_model');

        //$CI->uploader_model->getAdmin();
        ...
        $con = mysql_connect($hostname, $username, $password) 
          or die("Unable to connect to MySQL" . PHP_EOL);
        if($con) {
          mysql_select_db($db) or die(mysql_error().' xxx');
        } else {
          die('failed');
        }*/
    ...
    protected function set_additional_file_properties($file) {
        $file->deleteUrl = $this->options['script_url'].'uploader/upload'
            .$this->get_query_separator($this->options['script_url'])
            .$this->get_singular_param_name()
...
//MOA-insert
        ...
        if(!isset($files[0]->error)){
         $data = array('name' => $files[0]->name,
                       'size' => $files[0]->size,
                       'type' => $files[0]->type,
                       'url' => $files[0]->url,
                       'medium' => $files[0]->mediumUrl,
                       'thumbnail' => $files[0]->thumbnailUrl,
                    );
         $this->CI->uploader_model->insertImage($data);
        }
         //print_r($files[0]);
            ...
            if ($success) {
                foreach($this->options['image_versions'] as $version => $options) {
                    if (!empty($version)) {
                        ...
                        if (is_file($file)) {
//MOA-delete
                            $this->CI->uploader_model->deleteImageFromDB($file_name);
                            unlink($file);
                        }
                    }
                }
